...
load(file="./data/modulo2_es05_interpolation_deterministic.rda")
#load(file="./smpl.df.rda")
# raster temperatura
...
######################################################
library(raster)
temperature.rveneto.raster <- raster(temperature.rveneto)
temperature.rveneto.raster <- as(temperature.rveneto.raster, "SpatialGridDataFrame")
proj4string(temperature.rveneto.raster)<- CRS("EPSG:4326")
# check
gridded(temperature.rveneto.raster)<- TRUE

is.projected(temperature.rveneto.raster) 
##########################################################

coordinates(smpl.df) <- ~x+y
proj4string(smpl.df) <- CRS("EPSG:4326")
...
mv <- fit.variogram(v,vgm(range=1,model="Gau",nugget=1,psill=5))

plot(v,mv)

# by not naming the arguments it works otherwise it fails
#https://github.com/r-spatial/gstat/issues/69
krige(Temp~1,
      smpl.df,
      temperature.rveneto.raster,
      model=mv)


# automap 
# https://stackoverflow.com/questions/70101647/kriging-using-the-gstat-and-automap-packages-problems-when-copying-a-tutorial


library(automap)

v_autofit <- autofitVariogram(Temp~1,
                              smpl.df)
plot(v_autofit)
...
# however the usual variogram models are typically not non-negative definite on the sphere,
# and no appropriate models are available' (Edzer Pebesma on r-sig-geo).

# https://hypatia.math.ethz.ch/pipermail/r-sig-geo/2009-July/006078.html
