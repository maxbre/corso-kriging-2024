...
load(file="./data/pirotti_modulo2_es05_interpolation_deterministic.rda")

coordinates(smpl.df) <- ~x+y
proj4string(smpl.df) <- CRS("epsg:4326")
is.projected(smpl.df)

# raster temperatura
temperature <- terra::rast("./data/VenetoCorrectedMODIS_LST_Avg2017.tif")

# check
crs(temperature, proj=TRUE, describe=TRUE)
...
mv <- fit.variogram(v, vgm(range=1,model="Sph",nugget=1,psill=30 ) )

# The range is the distance after which the variogram levels off. 
# The physical meaning of the range is that pairs of points that are this distance 
# or greater apart are not spatially correlated. 
# The sill is the total variance contribution, or the maximum variability between pairs of points.

# original from pirotti
...
#coordinates(z) <- ~x+y
gridded(z) <- TRUE
proj4string(z) <- CRS("epsg:4326")
is.projected(z)

ok_predict <- predict(gOK, z)
